...
final class RecordedHarmonicaUnitTests: XCTestCase {
    ...
    func test_loop_whenItWraps_arrivesFromWhereItLeftOff() throws {
        ...
        let looping = RecordedHarmonica.withASmoothedLoop(
            recordedTone(seconds: 4),
            loop: loop,
            framesPerSecond: framesPerSecond
        )

        let wrap = abs(looping[loop.lowerBound] - looping[loop.upperBound - 1])
        let ordinary = abs(looping[loop.lowerBound] - looping[loop.lowerBound - 1])
    ...
    func test_loop_whenTheRecordingCannotSpareACrossfade_isLeftAlone() {
        let tone = recordedTone(seconds: 0.01)

        let looping = RecordedHarmonica.withASmoothedLoop(tone, loop: 1..<100, framesPerSecond: framesPerSecond)

        XCTAssertEqual(looping, tone)
    }
