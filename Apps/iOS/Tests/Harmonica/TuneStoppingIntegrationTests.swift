...
final class TuneStoppingIntegrationTests: XCTestCase {
    ...
    func test_demo_whenChosen_waitsBeforeTheFirstNote() async {
        let environment = TestEnvironment(pauseBeforeATune: .seconds(1))
        let screen = await environment.harmonicaScreen()

        screen.playTune(at: 0)

        XCTAssertEqual(environment.engine.soundedTones, [], "the menu has only just closed")
        XCTAssertEqual(screen.playable?.demo.isPlaying, true, "the stop button is there to cancel the wait")
        let sounded = await waitUntil { !environment.engine.soundedTones.isEmpty }
        XCTAssertTrue(sounded)
    }
