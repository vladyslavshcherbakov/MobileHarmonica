...
final class ReedSamplerUnitTests: XCTestCase {
    ...
    func test_renderHealth_whenMoreReedsRingThanTheBankHolds_countsTheStolenVoices() {
        let sampler = ReedSampler(samples: SineSamples.bank(), log: RecordingLog())
        for chord in 0..<4 {
            sampler.sound(tenTones(from: 200 + Double(chord) * 100), over: 0.05, everyReedSpeaksAgain: false)
        }

        RenderedSound.render(1, from: sampler)

        XCTAssertEqual(sampler.health.collect().stolenVoices, 8, "forty reeds asked of a bank of thirty-two")
    }

    func test_renderHealth_whenTheOutputPassesFullScale_countsTheClippedSamples() {
        let sampler = sounding(hertz: 440)

        RenderedSound.render(1, from: sampler, amplitude: 4)

        XCTAssertGreaterThan(sampler.health.collect().clippedSamples, 0)
    }

    func test_renderHealth_whenOneReedSoundsAtFullScale_clipsNothing() {
        let sampler = sounding(hertz: 440)

        RenderedSound.render(1, from: sampler)

        XCTAssertEqual(sampler.health.collect().clippedSamples, 0)
    }

    // MARK: - Helpers

    private func tenTones(from hertz: Double) -> [SoundingTone] {
        (0..<10).map { SoundingTone(hertz: hertz + Double($0) * 10, bendableSemitones: 0) }
    }
