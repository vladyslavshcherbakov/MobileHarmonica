...
final class NoteRowIntegrationTests: XCTestCase {
    ...
    func test_noteRow_whenTheDrawReedIsBentToItsLimit_namesTheBentNoteAndTheReedBehindIt() async {
        ...
        screen.touchShapingPad(heightAboveTheMiddle: 1, vibrato: 0)
    ...
    func test_noteRow_whenAFingerBelowTheMiddleOfThePadMeetsABendingReed_namesThePlainNote() async {
        let screen = await environment.harmonicaScreen()
        screen.touchShapingPad(heightAboveTheMiddle: -0.8, vibrato: 0.7)

        screen.touchStrip(at: 0.35, above: -0.3)

        XCTAssertEqual(screen.hole(4)?.note, "D5", "hole 4 draw bends, and the lower half of the pad does not bend it")
        XCTAssertEqual(screen.hole(4)?.effect, "")
    }
    ...
    func test_noteRow_whenTheBendRoundsToNoSemitone_namesOnlyTheNote() async {
        ...
        screen.touchShapingPad(heightAboveTheMiddle: 0.1, vibrato: 0)
