...
final class SettingsPresenterUnitTests: XCTestCase {
    ...
    func test_shapingPadPlacementChoices_sayWhichSideOfTheHolesThePadStands() {
        let state = SettingsPresenter().present(.atFirstLaunch)

        XCTAssertEqual(state.shapingPadPlacement.choices.map(\.name), ["Left of the holes", "Right of the holes"])
    }
}
