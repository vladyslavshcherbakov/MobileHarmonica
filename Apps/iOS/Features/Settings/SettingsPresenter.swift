...
struct SettingsPresenter {
    ...
    private static let placementLabel = "Shaping pad"
    private static let sizeLabel = "Shaping pad size"
    ...
    func present(_ settings: PlayerSettings) -> SettingsViewState {
        SettingsViewState(
            ...
            shapingPadPlacement: Self.placementState(settings.shapingPadPlacement),
            shapingPadSize: SettingsViewState.Size(label: Self.sizeLabel, fraction: settings.shapingPadSize.fraction)
        )
    }
    ...
    private static func placementState(_ placement: ShapingPadPlacement) -> SettingsViewState.Placement {
        SettingsViewState.Placement(
            label: placementLabel,
            chosen: placement,
            choices: ShapingPadPlacement.allCases.map { SettingsViewState.PlacementOption(id: $0, name: name(of: $0)) }
        )
    }
