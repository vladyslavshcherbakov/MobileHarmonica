...
struct SquareSizing {
    ...
    func side(for size: SquareSize) -> CGFloat {
        smallestSide + (largestSide - smallestSide) * size.fraction
    }

    func size(afterPinching size: SquareSize, by magnification: CGFloat) -> SquareSize {
        let range = largestSide - smallestSide
        guard range > 0 else { return size }

        return SquareSize(clamping: (side(for: size) * magnification - smallestSide) / range)
    }
}
