...
enum RecordedHarmonica {
    ...
    static func withASmoothedLoop(
        ...
        var framesWithABlendedTail = mono
        for step in 0..<width {
            ...
            framesWithABlendedTail[loop.upperBound - width + step] = leaving * cos(angle) + arriving * sin(angle)
        }
        return framesWithABlendedTail
    }
    ...
    private static func read(_ url: URL, appendingTo frames: inout [Float]) throws -> RecordedNote {
        ...
        let file = try openedFile(at: url, named: name)
        let recordedFrames = try monoFrames(of: file, named: name)
        let framesPerSecond = file.processingFormat.sampleRate
        let loop = try loopBounds(framesPerSecond: framesPerSecond, within: recordedFrames.count, named: name)
        let loopingFrames = withASmoothedLoop(recordedFrames, loop: loop, framesPerSecond: framesPerSecond)

        let start = frames.count
        frames.append(contentsOf: loopingFrames[0..<loop.upperBound])
        return RecordedNote(
            start: start,
    ...
    private static func openedFile(at url: URL, named name: String) throws -> AVAudioFile {
        do {
            return try AVAudioFile(forReading: url)
        } catch {
            throw RecordedHarmonicaError.undecodable(name, error)
        }
    }
