...
A score is a text file with the extension `.score`. The app reads the first one it finds here
and the demo button plays it; with none, the button plays the twelve bar blues built into
`DemoScore.swift`.

These files are not committed, the same as the WAV samples: what you write here is yours and
its rights are yours.
