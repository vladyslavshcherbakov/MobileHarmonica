...
public struct RichterTuning: Sendable {
    // MARK: - Public

    public init() {}

    public func note(for reed: Reed, in key: HarmonicaKey) -> MIDINote {
        note(for: reed).transposed(by: key.semitonesFromC)
    }

    public func bendableSemitones(for reed: Reed) -> Double {
    ...
    public func overbendableSemitones(for reed: Reed) -> Double {
