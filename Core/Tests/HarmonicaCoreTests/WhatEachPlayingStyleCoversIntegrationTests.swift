...
@MainActor
final class WhatEachPlayingStyleCoversIntegrationTests: XCTestCase {
    ...
    func test_oneFinger_whenTheCentreIsBelowTheLine_drawsEveryCoveredHole() {
        ...
        XCTAssertEqual(
            environment.engine.soundedTones.last?.map { $0.pitch.converted(to: .hertz).value.rounded() },
            [494, 587],
    ...
    private func playing(_ style: PlayingStyle) -> PlayHarmonicaUseCase {
