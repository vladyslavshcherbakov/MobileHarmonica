...
export class ReedSampler {
    ...
    private nextSample(ramp: GainRamp, vibrato: VibratoSettings, cupCoefficient: number): number {
        ...
        const gainPerReed = this.breathGain / this.airSpreadAcrossTheReeds()
        const swinging = this.nextVibrato(vibrato)

        let summedVoices = 0
        for (const voice of this.voices) {
            summedVoices += voice.nextSample(this.samples.frames, this.sampleRate, ramp, swinging.pitchRatio)
        }
        this.cupped += (summedVoices * gainPerReed * swinging.gain - this.cupped) * cupCoefficient
        return this.cupped
    }
...
class Voice {
    ...
    nextSample(frames: Float32Array, sampleRate: number, ramp: GainRamp, pitchRatio: number): number {
        const sampleAtItsGain = this.interpolated(frames) * this.gain
...
export function registerHarmonicaProcessor(): void {
    class HarmonicaProcessor extends AudioWorkletProcessor {
        private readonly sampler: ReedSampler
        private readonly amplitude: number
        private isWaitingToSpeak = false

        constructor(options?: { processorOptions?: unknown }) {
            super(options)
            const settings = (options?.processorOptions ?? {}) as { amplitude?: number }
            this.amplitude = settings.amplitude ?? 0.25
            this.sampler = new ReedSampler(silentBank, sampleRate)
        ...
        process(_inputs: Float32Array[][], outputs: Float32Array[][]): boolean {
            ...
            this.sampler.render(channel, this.amplitude)
        ...
        private reportSpeaking(channel: Float32Array): void {
            if (!this.isWaitingToSpeak) return
        ...
        private receive(message: WorkletMessage): void {
            switch (message.kind) {
                case 'samples':
                    this.sampler.useRecordings({ frames: message.frames, notes: message.notes })
                    return
                case 'sound':
                    this.sampler.sound(message.tones, message.crossfadeSeconds, message.everyReedSpeaksAgain)
                    this.isWaitingToSpeak = message.tones.length > 0
                    this.report({ kind: 'took', at: currentTime })
                    return
                case 'breathGain':
                    this.sampler.changeBreathGain(message.value)
                    return
                case 'bend':
                    this.sampler.changeBend(message.value)
                    return
                case 'vibrato':
                    this.sampler.changeVibrato(message.value)
                    return
                case 'cup':
                    this.sampler.cupHands(message.value)
                    return
                case 'ringDown':
                    this.sampler.ringDown()
                    return
                case 'damp':
                    this.sampler.damp(message.seconds)
                    return
            }
        }
    }
