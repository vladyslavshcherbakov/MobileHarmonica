import type { AudioEngineError, AudioEngine } from '../core/audioEngine.js'
import { AudioEngineFailure } from '../core/audioEngine.js'
import type { CoreTone, CoreToneChange } from '../core/coreAudio.js'
...
export class SampledAudioEngine implements AudioEngine {
    ...
    async prepare(): Promise<void> {
        try {
            ...
            throw new AudioEngineFailure(engineErrorFrom(failure), describe(failure))
        }
    }
...
function engineErrorFrom(failure: unknown): AudioEngineError {
    if (!(failure instanceof RecordingError)) return { kind: 'outputRefused' }

    return failure.fault === 'notListed'
        ? { kind: 'noRecordings' }
        : { kind: 'recordingUnreadable', recording: failure.recording }
}
